...
description: "Solution to the problem: KMP Algo"
tags:
  - dsadeck
  - strings
---
...
- Dry Run :
  ...
  - when needle ptr `i` is at index `4` and haystack ptr `j` is at index `4` and theres a mismatch
    ...
    - when there is a mismatch at `j = 4`, `lps [3] -> 2` says : _Resume checking from index 2, since the longest prefix suffix of the string of size is of length 2, it has already occurred once before, now resume checking from the 3rd character_.
